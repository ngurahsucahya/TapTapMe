...
def main():
    ...
    while True:
        ...
        for c in range(3):
            # Tentukan batasan atas, bawah, kiri, dan kanan untuk gambar yang di-overlay
            overlay_top = max(0, rect1_position[1])
            overlay_bottom = min(green_background.shape[0], rect1_position[1] + rect_size[1])
            overlay_left = max(0, rect1_position[0])
            overlay_right = min(green_background.shape[1], rect1_position[0] + rect_size[0])

            # Tentukan area yang sesuai di latar belakang untuk overlay
            bg_overlay_region = green_background[overlay_top:overlay_bottom, overlay_left:overlay_right, c]

            # Tentukan area gambar yang sesuai untuk overlay
            img_overlay_region = bola1_rgb[:overlay_bottom - overlay_top, :overlay_right - overlay_left, c]

            # Hitung overlay dengan mempertimbangkan alpha channel
            green_background[overlay_top:overlay_bottom, overlay_left:overlay_right, c] = (
                    img_overlay_region * bola1_alpha[:overlay_bottom - overlay_top, :overlay_right - overlay_left] +
                    bg_overlay_region * (
                                1.0 - bola1_alpha[:overlay_bottom - overlay_top, :overlay_right - overlay_left])
            )
        # overlay sama bg
        lingkaranBenar_resized = cv2.resize(lingkaranBenar, (220, 270))  # Resize lingkaranBenar fixed size
